...
#define CONNECT(from, from_port, to, to_port) rc = nz_graph_connect(graph, from, from_port, to, to_port); if (rc != NZ_SUCCESS) { err_line = LINESTRING ", connect: " #from " " #from_port " " #to " " #to_port; goto err; }
    ...
    ADD_BLOCK("n_beats_in_melody", "constant(real,4.0)");
    ADD_BLOCK("n_beat_delay", "constant(real,-0.75)");
    ADD_BLOCK("beat_in_melody_delay", "sum");
    ADD_BLOCK("beat_in_melody", "mod");

    ADD_BLOCK("cream_smf", "midireader(cream.midi)");
    ...
    ADD_BLOCK("vol1", "constant(real,0.5)");
    ADD_BLOCK("vol2", "constant(real,0.8)");
    ...
    ADD_BLOCK("soundcard", "wavfileout(cream.wav)");
    ...
    CONNECT("ruler", "out 4", "beat_in_melody_delay", "b");
    CONNECT("n_beat_delay", "out", "beat_in_melody_delay", "a");
    CONNECT("beat_in_melody_delay", "out", "beat_in_melody", "a");
    CONNECT("n_beats_in_melody", "out", "beat_in_melody", "b");

    CONNECT("beat_in_melody", "out", "cream_smf", "in");
    CONNECT("cream_smf", "out", "melody", "in");
    ...
    CONNECT("ruler", "out 2", "drums_smf", "in");
