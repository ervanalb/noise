...
nz_rc run()
{
    ...
    if((rc = nz_context_create(&context)) == NZ_SUCCESS)
    {
        ...
        if((rc = nz_graph_create(context, &graph)) == NZ_SUCCESS)
        {
            ...
            rc = nz_graph_add_block(graph, "sound2", "wave(saw)"); if(rc != NZ_SUCCESS) goto err;
            rc = nz_graph_add_block(graph, "freq1", "constant(real,440)"); if(rc != NZ_SUCCESS) goto err;
            rc = nz_graph_add_block(graph, "freq2", "constant(real,554.4)"); if(rc != NZ_SUCCESS) goto err;
            ...
            rc = nz_graph_add_block(graph, "soundcard", "pa"); if(rc != NZ_SUCCESS) goto err;
