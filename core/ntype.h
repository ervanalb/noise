...
// nz_obj_p is a void pointer. It points to the actual data of a noise object. Its contents vary based on
...
// the nz_obj_p can be safely cast to a double* and examined or mutated.

typedef void* nz_obj_p;

// Every noise object has a type. nz_type_p is also a void pointer. It points to any additional data
...
struct nz_typeclass {
    ...
    nz_rc (*type_str)         (const nz_type_p type_p, char ** string);
...
// type_create is a function which creates a type from a typeclass. It populates type_p with
...
// it in type_p.
// If type_create returns NZ_SUCCESS, then it has allocated and populated the type_p. The
...
// type_p is undefined and no memory needs to be freed later.

// The next four methods operate on types. They take the type_p data pointer as their first
// argument.
...
// type_p's that are known to be of the same typeclass. To check equality of both typeclass
...
// type_init_obj populates an object with data parsed out of a string. The obj_p must
...
// type_copy_obj populates an object with the data from another object. The source obj_p
